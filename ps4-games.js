// PS4 Games Page Functionality

const ITEMS_PER_PAGE = 25;
let currentPage = 1;
let filteredGames = [];

// Get all PS4 games from the products database
function getPS4Games() {
    return Object.values(products).filter(product => 
        product.category === "Playstation 4 Game"
    );
}

// Extract unique genres from PS4 games
function getUniqueGenres() {
    const ps4Games = getPS4Games();
    const genres = new Set(ps4Games.map(game => game.genre));
    return Array.from(genres).sort();
}

// Filter and sort products
function filterAndSortProducts() {
    let filteredGames = getPS4Games();
